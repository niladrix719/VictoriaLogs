...
func (pf *pipeFilter) initFilterInValues(cache *inValuesCache, getFieldValues getFieldValuesFunc) (pipe, error) {
	fNew, err := initFilterInValuesForFilter(cache, pf.f, getFieldValues)
	if err != nil {
		return nil, err
	}
	pfNew := *pf
