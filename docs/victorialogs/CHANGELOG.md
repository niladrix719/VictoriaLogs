...
* FEATURE: [LogsQL](https://docs.victoriametrics.com/victorialogs/logsql/): support `sort by (...)` (aka `order by (...)`) for the [`values` stats function](https://docs.victoriametrics.com/victorialogs/logsql/#values-stats), consistently with the [`json_values` stats function](https://docs.victoriametrics.com/victorialogs/logsql/#json_values-stats). This allows ordering the collected values by the given fields of the matching log entries and selecting top N values when combined with `limit N`. See [#1426](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1426).
