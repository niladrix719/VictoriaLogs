...
* BUGFIX: [LogsQL](https://docs.victoriametrics.com/victorialogs/logsql/): fix [`field_names` pipe](https://docs.victoriametrics.com/victorialogs/logsql/#field_names-pipe) and `/select/logsql/field_names` HTTP API not respecting the query filter. Previously, they could return fields missing in all the logs matching the query. See [#1745](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1745).
